...
def evaluate_at_threshold(model, image_files, labels_dir, conf_threshold, iou_threshold=0.5):
    """
    在给定置信度阈值下评估模型。
    返回: (total_tp, total_fn, total_fp, num_gt_boxes, image_details)
    """
    total_tp = 0
    total_fn = 0
    total_fp = 0
...
def evaluate_overkill(model, negative_image_dir, conf_threshold):
    """
    评估过杀率：在纯负样本（无缺陷）上统计误报情况。
    返回: (误报图像数, 总负样本数, 总误报框数)
    """
...
def evaluate_with_tta(model, image_files, labels_dir, conf_threshold, iou_threshold=0.5):
    """
    使用Test-Time Augmentation (TTA) 评估。
    TTA对每张图片做多尺度+翻转推理，取平均结果，通常能提升召回率。
    """
    total_tp = 0
    total_fn = 0
    total_fp = 0
...
def main():
    ...
    if manual_conf_threshold is None:
        ...
        for conf in conf_thresholds:
            tp, fn, fp, total_gt = evaluate_at_threshold(
                model, image_files, test_labels_dir, conf, iou_threshold
            )
            ...
            beta = 2
            f_beta = (1 + beta**2) * recall * precision / (beta**2 * precision + recall) if (recall + precision) > 0 else 0
        ...
        for conf, rec, prec, f1, f2, tp, fn, fp in sweep_results:
            ...
            print(f"{conf:>8.3f}  {rec:>8.4f}  {prec:>10.4f}  {f1:>8.4f}  {f2:>8.4f}  {tp:>5}  {fn:>5}  {fp:>5}{marker}")
    ...
    tp, fn, fp, total_gt = evaluate_at_threshold(
        model, image_files, test_labels_dir, eval_conf, iou_threshold
    )
    recall = tp / (tp + fn) if (tp + fn) > 0 else 0
    precision = tp / (tp + fp) if (tp + fp) > 0 else 0
    f1 = 2 * recall * precision / (recall + precision) if (recall + precision) > 0 else 0
    ...
    if use_tta:
        ...
        tp_tta, fn_tta, fp_tta, _ = evaluate_with_tta(
            model, image_files, test_labels_dir, eval_conf, iou_threshold
        )
        recall_tta = tp_tta / (tp_tta + fn_tta) if (tp_tta + fn_tta) > 0 else 0
        precision_tta = tp_tta / (tp_tta + fp_tta) if (tp_tta + fp_tta) > 0 else 0
        f1_tta = (2 * recall_tta * precision_tta / (recall_tta + precision_tta)
                  if (recall_tta + precision_tta) > 0 else 0)
    ...
    if os.path.exists(negative_images_dir):
        fp_img, total_neg, total_fp_boxes = evaluate_overkill(
            model, negative_images_dir, eval_conf
        )
